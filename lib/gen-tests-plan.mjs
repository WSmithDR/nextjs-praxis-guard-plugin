...
export function extractComponentSignature(ts, targetFile) {
  ...
  let sf;
  try { sf = ts.createSourceFile(targetFile, text, ts.ScriptTarget.Latest, true); } catch { return null; }
  const isTsx = /\.(tsx|jsx)$/.test(targetFile);
  const interfaces = new Map();
  const declaredFns = new Map();   // name -> fnNode (toda función/const-fn del archivo, exportada o no)
  let best = null;
  ...
  for (const st of sf.statements) {
    ...
    if (ts.isFunctionDeclaration(st) && st.name) declaredFns.set(st.name.text, st);
    if (ts.isVariableStatement(st)) {
      for (const d of st.declarationList.declarations) {
        if (d.name && ts.isIdentifier(d.name) && d.initializer && (ts.isArrowFunction(d.initializer) || ts.isFunctionExpression(d.initializer))) {
          declaredFns.set(d.name.text, d.initializer);
        }
      }
    }
    ...
    if (ts.isExportAssignment(st) && !st.isExportEquals) {
      ...
      if (ts.isIdentifier(e)) record(e.text, 'default', null);   // `export default Foo` -> fnNode se resuelve abajo
      else if (ts.isArrowFunction(e) || ts.isFunctionExpression(e)) record(basename(targetFile, extname(targetFile)), 'default', e);
    }
  }
  if (!best) return null;
  // `export default Foo` con Foo definido en el archivo (patrón común) -> resolver su fnNode.
  if (!best.fnNode && declaredFns.has(best.name)) best.fnNode = declaredFns.get(best.name);
