...
const KNOWN_RULES = ['secrets', 'hardcoded-data', 'forbidden-imports', 'file-responsibility', 'untranslated-text',
  'single-component-per-file', 'inline-mapped-component', 'excessive-usestate', 'descriptive-component-names', 'thin-route-pages', 'tailwind-content-coverage',
...
const NUMERIC_KEYS = ['maxLines', 'mixedSignalsLines', 'minElements', 'minProps', 'minRepeats', 'minLiterals', 'minMembers', 'maxClasses', 'minFiles', 'minLen', 'minTags', 'maxStructuralTags', 'max'];
          ...
          if (!Array.isArray(rule.list)) {
            ...
            rule.list.forEach((e, i) => {
              if (!isObject(e) || typeof e.module !== 'string') {
                ...
                if ('message' in e && typeof e.message !== 'string') errors.push(`rules.forbidden-imports.list[${i}].message debe ser string`);
                if ('allowDirs' in e && !isStringArray(e.allowDirs)) errors.push(`rules.forbidden-imports.list[${i}].allowDirs debe ser array de strings`);
              }
            });
          }
        }
        if (id === 'file-responsibility' && 'overrides' in rule) {
          if (!Array.isArray(rule.overrides)) {
            errors.push('rules.file-responsibility.overrides debe ser un array');
          } else {
            rule.overrides.forEach((e, i) => {
              if (!isObject(e) || typeof e.glob !== 'string') errors.push(`rules.file-responsibility.overrides[${i}] debe tener "glob" (string)`);
              else for (const k of ['maxLines', 'mixedSignalsLines']) {
                if (k in e && typeof e[k] !== 'number') errors.push(`rules.file-responsibility.overrides[${i}].${k} debe ser número`);
              }
            });
          }
        }
        if (id === 'single-component-per-file' && 'ignore' in rule && !isStringArray(rule.ignore)) {
          errors.push('rules.single-component-per-file.ignore debe ser array de strings');
        }
        if (id === 'descriptive-component-names') {
          for (const k of ['blocklist', 'allow']) {
            if (k in rule && !isStringArray(rule[k])) errors.push(`rules.descriptive-component-names.${k} debe ser array de strings`);
          }
        }
