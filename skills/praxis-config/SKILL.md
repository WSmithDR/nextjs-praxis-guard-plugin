...
2. **Preguntá al usuario** (en Claude Code podés usar la UI de opciones; en otros CLIs,
   ...
   - **Reglas TypeScript idiomático (AST, `--deep`):** `prefer-satisfies` (`minProps`) y
     `as-const-opportunity` (prendidas); `prefer-discriminated-union` (`minMembers`) y
     `prefer-branded-type` (`pattern`) — **experimentales, `enabled:false` por default**, prendelas a demanda.
   ...
   - **Tailwind theme-aware (AST, `--deep`):** `tailwind-arbitrary-has-token` (prendida; sugiere el
     ...
     (experimental, off). Parsean `tailwind.config` estáticamente —o el `@theme` del CSS en v4—; solo tokens del proyecto.
